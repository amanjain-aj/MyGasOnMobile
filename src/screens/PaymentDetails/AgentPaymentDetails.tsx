...
const AgentPaymentDetails: any = ({
  ...
  useEffect(() => {
    ...
    AsyncStorage.multiGet(['API_TOKEN', 'USER_ID'], (err, items) => {
      if (err) {
        ...
        setToken(items[0][1]);
        getAgentById(route.params.id, items[0][1])
          .then(res => {
            setLoading(false);
  ...
  const UpdatePayment = () => {
    ...
    // console.log("NEW DETAILS",body)
      ...
      //create a mapping
      UpdateAgentProfile(details.agentId, body, token)
        .then(res => {
          setLoading(false)
          if (res.status === 200) {
            navigation.goBack();
