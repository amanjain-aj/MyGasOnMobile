  ...
  useEffect(() => {
    ...
    if (route.params.type === 'edit') {
      ...
      AsyncStorage.multiGet(['API_TOKEN', 'USER_ID'], (err, items) => {
        ...
        settoken(items[0][1]);
        getAgentStoreById(route.params.storeId, items[0][1])
          .then(res => {
        ...
        if (valid) {
          ...
          AsyncStorage.multiGet(['API_TOKEN', 'USER_ID'], (err, items) => {
            ...
            if (route.params.type === 'edit') {
              ...
              updateAgentStore(route.params.storeId, token, body)
                .then(res => {
                  if (res.status === 200) {
                    navigation.goBack();
                    setloader(false);
              ...
              createNewAgentStore(items[0][1], body)
                .then(res => {
                  setloader(false);
                  navigation.goBack();
                })
      ...
      <PostAuthWrapper
        ...
        <>
          <Input
            ...
            iconName=""
            placeholder={I18n.t(
              'customerStore.store_input_gowdown_name_placeholder',
            )}
            disabled={false}
            error={false}
            mode="flat"
          ...
          <Input
            ...
            iconName=""
            placeholder={I18n.t(
              'customerStore.store_input_address_placeholder',
            )}
            disabled={false}
            error={false}
            mode="flat"
