...
const AgentAddItem: any = ({
  ...
  useEffect(() => {
    ...
    AsyncStorage.multiGet(['API_TOKEN', 'USER_ID'], (err, items) => {
      ...
      console.log('ID cbcbcbc' + route.params.custId);
      setToken(items[0][1]);
      setId(items[1][1]);
      GetItemListing(items[0][1], items[1][1], 1)
        .then(res => {
          ...
          if (route.params.type === 'edit') {
            GetContractByIDAgent(items[0][1], route.params.contractId)
              .then(res => {
  ...
  const addContract = () => {
    ...
    if (route.params.type === 'edit') {
      UpdateContractForAgent(token, body, route.params.contractId)
        .then(res => {
          setloading(false);
      ...
      AddContractForAgent(token, body)
        .then(res => {
          setloading(false);
          console.log(res.status);
          if (res.status === 201) {
