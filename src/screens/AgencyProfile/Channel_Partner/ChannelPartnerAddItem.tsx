...
const ChannelPartnerAddItem: any = ({
  navigation,
  route,
}: {
  navigation: any;
      ...
      GetItemListing(items[0][1], items[1][1], 1)
        .then(res => {
          ...
          if (route.params.type === 'edit') {
            setloading(true);
            // console.log(token)
            GetContractByIDChannelPartner(items[0][1], route.params.contractId)
              .then(res => {
  ...
  const addContract = () => {
    ...
    const body = {
      ...
      remarks: 'No Remarks',
      channelPartner: {
        channelPartnerId: route.params.custId,
      },
      itemPrice: {
        priceId: issuedItem,
      },
    ...
    if (route.params.type === 'edit') {
      UpdateContractForChannelPartner(token, body, route.params.contractId)
        .then(res => {
          setloading(false);
      ...
      AddContractForChannelPartner(token, body)
        .then(res => {
          setloading(false);
          console.log(res.status);
          if (res.status === 201) {
...
export default ChannelPartnerAddItem;
